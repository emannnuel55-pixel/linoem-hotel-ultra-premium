...
// ── GASTOS ────────────────────────────────────────────────────────────────────
app.get('/v1/expenses',role('SUPER_ADMIN','MANAGER','FINANCE'),safe(async(_,res)=>res.json({items:(await q(`SELECT x.*,e.name AS created_by_name FROM expenses x LEFT JOIN employees e ON e.id=x.created_by ORDER BY occurred_on DESC,created_at DESC`)).rows})));
app.post('/v1/expenses',role('SUPER_ADMIN','MANAGER','FINANCE'),safe(async(req,res)=>{const d=z.object({category:z.string().min(2).max(80),kind:z.enum(['FIXED','VARIABLE','ASSET']),description:z.string().min(3).max(500),amount:z.number().positive().max(100000000),occurredOn:z.string().date()}).parse(req.body);res.status(201).json((await q('INSERT INTO expenses(category,kind,description,amount,occurred_on,created_by) VALUES($1,$2,$3,$4,$5,$6) RETURNING *',[d.category,d.kind,d.description,d.amount,d.occurredOn,req.employee.id])).rows[0])}));
